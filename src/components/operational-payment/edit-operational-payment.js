...
import { editOperationalPayment } from '../../services/api/operational-payment';

export const EditOperationalPayment = ({ operationalPayment }) => {
  const router = useRouter();

  const [info, setInfo] = useState(undefined);
  const [errMessage, setErrMessage] = useState(undefined);
  const [amount, setAmount] = useState(operationalPayment.amount);
  const [discount, setDiscount] = useState(operationalPayment.discount);
  const [receipt, setReceipt] = useState(operationalPayment.receiptUrl);

  const formik = useFormik({
    initialValues: {
      courseId: operationalPayment.courseId,
      descriptions: operationalPayment.descriptions,
      transactionDate: operationalPayment.transactionDate,
      status: operationalPayment.status,
      receiptUrl: operationalPayment.receiptUrl,
    },
    ...
    onSubmit: (values, action) => {
      ...
      editOperationalPayment(operationalPayment.id, values)
        .then((res) => {
          setInfo(res);
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Card>
        <CardHeader
          subheader="Fill out this form for edit operational payment"
          title="Edit Operational Payment"
        />
        <Divider />
        <CardContent>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item lg={6} md={6} xs={12}>
              ...
              <FormControl sx={{ marginY: 2 }} fullWidth variant="outlined">
                <InputLabel id="select-operational-payment-status">
                  Operational Payment Status
                </InputLabel>
                <Select
                  labelId="select-operational-payment-status"
                  id="select-operational-payment-status"
                  value={formik.values.status}
                  label="Operational Payment Status"
                  onChange={formik.handleChange}
                  name="status"
                >
                  <MenuItem disabled key="" value="">
                    --- Select Operational Payment Status ---
                  </MenuItem>
